...
def test_event_cart_fixes():
    """Test the fixed event cart logic."""
    ...
    try:
        ...
        print(f"✅ Cart item fields: {list(cart_item_1.__dict__.keys())}")
    ...
    for item in all_items:
        print(f"  - Item {item.id}: {item.product_type} - {item.quantity} - ${item.total_price}")
        if item.product_type == 'event':
            print(f"    Seats: {len(item.booking_data.get('seats', []))}")
            print(f"    Performance: {item.booking_data.get('performance_id')}")
            print(f"    Ticket Type: {item.booking_data.get('ticket_type_id')}")
    
    print("\n✅ All tests completed successfully!")

if __name__ == "__main__":
    test_event_cart_fixes() 