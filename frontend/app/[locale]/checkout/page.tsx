...
export default function CheckoutPage() {
  ...
  }, [authChecked, authLoading, isAuthenticated, router, locale]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!customerData) {
        throw new Error(t('customerDataNotFound'));
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || t('orderCreationError'));
      }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            {authChecked && !isAuthenticated ? t('redirecting') || 'Redirecting...' : t('loading') || 'Loading...'}
          </p>
        </div>
      </div>
    );
                ...
                <div className="space-y-4 mb-6">
                  {items.map((item: CartItem, index) => (
                    <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-600 flex-shrink-0 relative">
                        <OptimizedImage
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">{t('whatsappPayment')}</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {t('whatsappPaymentDescription')}
                      </p>
                    </div>
                  </div>
                </div>
