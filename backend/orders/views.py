...
class OrderActionView(APIView):
    ...
    def get(self, request, order_number, action):
        ...
        try:
            ...
            if action == 'receipt':
                ...
                return self._get_invoice(order, request)
            else:
                return Response(
    ...
    def _get_receipt(self, order, request):
        ...
        try:
            ...
            # Get language from request (query param, header, or default to 'en')
            language = request.GET.get('lang') or request.GET.get('language') or \
                      request.headers.get('Accept-Language', 'en').split(',')[0].split('-')[0].lower()
            
            # Validate language (fallback to 'en' if invalid)
            if language not in ['en', 'fa', 'ar']:
                language = 'en'

            # Generate PDF receipt using the same service as invoice with language support
            pdf_data = pdf_generator.generate_receipt(order, language=language)
    ...
    def _get_invoice(self, order, request):
        ...
        try:
            ...
            # Generate PDF invoice with language support
