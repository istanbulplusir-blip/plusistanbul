@echo off
REM Test script for production-like development environment
REM This script tests all services and endpoints
...
curl -s -I http://localhost:3000 | findstr "200 OK" || echo "Frontend check failed"
...
curl -s http://localhost:8000/api/v1/tours/ | findstr "results" || echo "Tours API failed"

echo.
echo Testing events API...
curl -s http://localhost:8000/api/v1/events/ | findstr "results" || echo "Events API failed"

echo.
echo Testing shared API...
curl -s http://localhost:8000/api/v1/shared/ | findstr "results" || echo "Shared API failed"
